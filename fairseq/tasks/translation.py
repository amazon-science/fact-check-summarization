...
def load_langpair_dataset(
    ...
    if load_cls_labels:
        ...
        if indexed_dataset.dataset_exists(cls_labels_path, impl=dataset_impl):
            cls_dataset = data_utils.load_indexed_dataset(cls_labels_path, None, dataset_impl)
            if truncate_source:
    ...
    if load_cls_indices:
        ...
        if indexed_dataset.dataset_exists(cls_indices_path, impl=dataset_impl):
            cls_indices_dataset = data_utils.load_indexed_dataset(cls_indices_path, None, dataset_impl)

    sample_weights = None
    if load_sample_weights:
        weights_file = os.path.join(data_path, '{}.{}-{}.weights.npy'.format(split, src, tgt))
        assert os.path.exists(weights_file)
        with open(weights_file, 'rb') as f:
            sample_weights = np.load(f)
        logger.info('Loaded {} weights from {}'.format(len(sample_weights), weights_file))
