...
def parse_cls_indices(line):
    """
    Parses a single line from the cls_indices file.

    Args:
        line (str): String containing the alignment of the format:
            <start_idx_1>-<end_idx_1>-<label_1> <start_idx_2>-<end_idx_2>-<label_2> ..
            <start_idx_m>-<end_idx_m>-<label_m>. All indices are 0 indexed.

    Returns:
        torch.IntTensor: packed alignments of shape (3 * m).
    """
    start_end_labels = line.strip().split()
    parsed_indices = torch.IntTensor(3 * len(start_end_labels))
    for idx, t in enumerate(start_end_labels):
        start_idx, end_idx, label = t.split('-')
        parsed_indices[3 * idx] = int(start_idx)
        parsed_indices[3 * idx + 1] = int(end_idx)
        parsed_indices[3 * idx + 2] = int(label)
